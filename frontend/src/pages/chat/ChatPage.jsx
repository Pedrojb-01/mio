...
function MessageBubble({ role, content, isStreaming }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start ml-4'}`}>
      <div className={`
        group relative max-w-[85%] px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap break-words
        ${isUser
        ...
        {!isUser && !isTyping && content && (
          <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100">
            ...
            {copied ? (
              ...
              {!copied && (
                <span className="absolute top-full left-1/2 -translate-x-1/2 mt-1.5
                  px-1.5 py-0.5 rounded text-xs bg-accent text-white whitespace-nowrap
                  opacity-0 group-hover/copy:opacity-100 transition-opacity duration-150
                  pointer-events-none">
                  Copy
                </span>
              )}
